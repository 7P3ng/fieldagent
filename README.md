...
| **Cross-model finding (the interesting one)** | The "agentic chunking lift" is **model-specific, not real**. On DeepSeek, chunked extraction beats single-shot by +0.45 F1 — but that gap is an artifact of DeepSeek's single-shot response **truncating** under reasoning-token pressure (17/20 hit the cap). On **Claude Sonnet**, which finishes its one-pass response, single-shot (**F1 0.567**) **ties** the chunked pipeline (0.555) — chunking lift **−0.01** (CIs overlap). Cross-model validation caught what a single-model eval would have shipped as a headline. See [cross-model results](evals/results/cross_model_claude.json) + [writeup §5](docs/writeup.md). |
